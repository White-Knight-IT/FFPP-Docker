...
envsubst '${FFPP_DOMAIN} ${NGINX_HTTP_PORT}' < /root/config/nginx-cert-bootstrap.template > /etc/nginx/sites-available/$FFPP_DOMAIN
...
if test -e "/etc/letsencrypt/live/$FFPP_DOMAIN/privkey.pem"; then
    envsubst '${FFPP_DOMAIN} ${NGINX_HTTP_PORT} ${NGINX_HTTPS_PORT}' < /root/config/nginx-final.template > /etc/nginx/sites-available/$FFPP_DOMAIN
fi
